...
def is_capacity_feasible(route, capacity):
    total_demand = sum(node.q for node in route)
    return total_demand <= capacity



# Function to check if adding a new node to the route is feasible based on time windows
def is_time_feasible(route, distances):
...
# Function to check if a route is feasible based on both capacity and time
